...
/* set function emit loc, name == func name, emitLoc == to setting function location */
void st_set_emit_loc(char *name, int emitLoc);

/* get function emit loc, name == func name, return value == function's emit location */
int st_get_emit_loc(char *name);

/* get variable's location. name == variable name, return value == variable's relative location */
